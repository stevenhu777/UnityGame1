  ...
  float damage = 1;  
    ...
    void FixedUpdate()
    {
        if (state == PlayerState.Normal)
        {
            ...
            if (Hp == 0)
            {
                ...
                if (attackItem!=null)
                {
                    attackItem.GetComponent<Item>().team = PlayerTeam.Null;
                }
            }
        }
    ...
    void Attack()
    {
        if (attackItem != null && attackItem.GetComponent<Item>().state == Item.State.OnPlayer)
        {
            ...
            {
                if (attackItem.GetComponent<Item>().type == ItemType.Firebomb&&backBombArea!=null)
                {
                    attackItem.GetComponent<Firebomb>().SetAttackData(transform.position,backBombArea.transform.position,2f,7f);
                    attackItem.transform.GetChild(0).GetComponent<explosionRange>().damageCofficient = damageCofficient;
                    Destroy(backBombArea);
                    holdBomb = false;
                }
                else
                {
                    attackItem.GetComponent<Item>().AttackVelocity(attackVelocity);
                    attackItem.GetComponent<Item>().damageCofficient = damageCofficient;
